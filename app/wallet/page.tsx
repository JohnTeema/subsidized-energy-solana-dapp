...
const txIcons: Record<string, React.ReactNode> = {
  mint: <Zap size={14} className="text-amber-400" />,
...
const txColors: Record<string, string> = {
  mint: "bg-amber-500/10 border-amber-500/20",
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A]">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-24 pb-16">
        ...
        {/* Wallet address */}
        <div className="glass border border-white/[0.08] rounded-2xl p-5 mb-6 flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="flex-1 min-w-0">
            <p className="text-white/40 text-xs font-medium uppercase tracking-wider mb-1">Wallet Address</p>
            <p className="text-white font-mono text-sm truncate">{address || "Not connected"}</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handleCopy}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white/[0.05] border border-white/[0.08] text-white/60 text-xs hover:bg-white/[0.1] hover:text-white transition-all"
            >
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            {/* SUB */}
            <div className="glass border border-amber-500/20 bg-amber-500/[0.02] rounded-2xl p-6">
              ...
              <div className="mt-4 pt-4 border-t border-white/[0.06] flex items-center gap-2 text-xs text-white/25">
              ...
              <form onSubmit={handleSend} className="flex flex-col gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder-white/20 text-sm focus:outline-none focus:border-amber-500/40 transition-all"
                    required
                  />
                </div>
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-white placeholder-white/20 text-sm focus:outline-none focus:border-amber-500/40 transition-all pr-16"
                      required
                    />
                    <button
                      type="button"
                      onClick={() => setAmount(String(mockStats.sreBalance))}
                      className="absolute right-2 top-1/2 -translate-y-1/2 px-2 py-0.5 rounded text-amber-400 text-xs font-medium hover:bg-amber-500/10 transition-colors"
                    >
                      MAX
                    </button>
                  </div>
                ...
                <button
                  type="submit"
                  disabled={sending}
                  className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 text-black font-semibold text-sm hover:opacity-90 disabled:opacity-50 transition-all"
                >
                  {sending ? (
        ...
        {/* Transaction history */}
        <div className="mt-6 glass border border-white/[0.08] rounded-2xl p-6">
            ...
            {mockTransactions.map((tx) => (
              ...
              >
                ...
                <div className="text-right flex-shrink-0">
                  <p className={`text-sm font-semibold ${tx.type === "transfer" ? "text-red-400" : "text-emerald-400"}`}>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="p-1.5 rounded-lg text-white/15 hover:text-white/40 transition-colors flex-shrink-0"
                >
                  <ExternalLink size={12} />
                </a>
              </div>
