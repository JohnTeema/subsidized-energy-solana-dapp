...
function InvertersContent() {
  ...
  // inverters that haven't polled in 24h+
  const staleThreshold = new Date(Date.now() - 24 * 3_600_000).toISOString();
  const staleInverters = MOCK_INVERTERS.filter((i) => i.lastPoll < staleThreshold);

  const filtered = useMemo(() => {
    const q = search.toLowerCase();
    return MOCK_INVERTERS.filter((i) => {
  ...
  const activeCount = MOCK_INVERTERS.filter((i) => i.status === "active").length;
  const disconnectedCount = MOCK_INVERTERS.filter((i) => i.status === "disconnected").length;
  const errorCount = MOCK_INVERTERS.filter((i) => i.status === "error").length;
  ...
  const COLS: { key: SortKey; label: string }[] = [
    ...
    { key: "userWallet", label: "Wallet" },
    { key: "brand", label: "Brand" },
    { key: "status", label: "Status" },
    { key: "lastPoll", label: "Last Poll" },
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
        <div className="rounded-2xl p-4 border border-teal-500/[0.1] bg-[#111827]/60">
          ...
          <p className="text-2xl font-bold text-white">{MOCK_INVERTERS.length}</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[750px]">
            ...
            <tbody className="divide-y divide-teal-500/[0.06]">
              {pageData.map((inv) => {
                ...
                return (
                  <tr key={inv.id} className={`transition-colors ${inv.status === "error" ? "bg-red-500/[0.02]" : ""} hover:bg-teal-500/[0.03]`}>
                    <td className={td}>
                      {inv.userEmail ?? <span className="text-white/20 italic">no email</span>}
                    </td>
                    <td className={`${td} font-mono text-xs`}>
                      {inv.userWallet.slice(0, 8)}…{inv.userWallet.slice(-4)}
                    </td>
                    ...
                    <td className={`${td} ${isStale ? "text-amber-400/80" : ""}`}>
                      {timeSince(inv.lastPoll)}
                      {isStale && <span className="ml-1 text-[10px] text-amber-400/60">⚠</span>}
                    </td>
              ...
              {pageData.length === 0 && (
                <tr>
                  <td colSpan={COLS.length} className="py-12 text-center text-white/25 text-sm">No inverters match these filters</td>
                </tr>
              )}
            </tbody>
          </table>
